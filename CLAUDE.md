...
- 내용: `_pages/` (about=홈, research, people, publications, news), `_data/members.yml`, `_bibliography/papers.bib`, `_news/`
- 설정: `_config.yml` (url/baseurl, 기능 플래그, 폰트 URL, scholar 저자 강조)
- 로컬 오버라이드(gem 파일을 같은 경로로 덮음): `_sass/_variables.scss`, `_sass/_themes.scss`, `_sass/_typography.scss`
...
- 한/영 병기: 영문 단락 뒤에 `<p class="ko">…</p>` 로 한글 단락.
