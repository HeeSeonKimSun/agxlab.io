...
- 내용: `_pages/` (about=홈, research, projects, people, publications, news, pi=PI 개인 이력), `_data/members.yml`, `_data/projects.yml`, `_bibliography/papers.bib`, `_news/`
- 설정: `_config.yml` (url/baseurl, 기능 플래그, 폰트 URL, scholar 저자 강조)
- 로컬 오버라이드(gem 파일을 같은 경로로 덮음): `_sass/_variables.scss`, `_sass/_themes.scss`, `_sass/_typography.scss`, `_includes/footer.liquid`
...
- 진행 중인 건(넥센·서울대 협업 등)은 한 문장 수준으로만 쓰고 예산·상대 이름·저널명을 적지 않는다. 과장 금지.
